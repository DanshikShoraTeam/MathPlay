...
def populate():
    """
    Дерекқорды тестілік деректермен толтырады.
    Бір рет қолмен іске қосылады: python populate_db.py
    """

    # admin пайдаланушысын жасайды (егер жоқ болса)
    if not User.objects.filter(username='admin').exists():
        admin = User.objects.create_superuser('admin', 'admin@example.com', 'admin123')
    else:
        admin = User.objects.get(username='admin')

    # Алгебра бойынша Quiz ойыны
    ...
    # Көбейту кестесі бойынша Froggy ойыны
    Game.objects.get_or_create(
    ...
    # Формулаларды жұптастыру — Memory ойыны
    Game.objects.get_or_create(
    ...
    print('Дерекқор сәтті толтырылды!')
    print('admin пайдаланушысы: admin | Пароль: admin123')
    print('3 ойын қосылды: Quiz, Froggy, Memory')
